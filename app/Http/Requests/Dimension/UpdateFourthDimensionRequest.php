...
class UpdateFourthDimensionRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        $rules = [
            ...
            'name' => [
                'required',
                Rule::unique('fourth_dimensions', 'name')
                    ->whereNot('id', $this->route('fourthDimension')->id),
            ],
            'raw_data' => ['required']
        ];

        if ($this->request->get('dimension_parent_id') != '' && $this->request->get('dimension_parent_id') > 0) {
            array_push($rules['dimension_parent_id'], Rule::exists('fourth_dimensions', 'id'));
        } else {
