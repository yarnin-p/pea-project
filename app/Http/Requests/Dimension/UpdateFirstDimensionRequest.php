...
class UpdateFirstDimensionRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        $rules = [
            ...
            'name' => [
                'required',
                Rule::unique('first_dimensions', 'name')
                    ->whereNot('id', $this->route('firstDimension')->id),
            ],
            'raw_data' => ['required']
        ];

        if ($this->request->get('dimension_parent_id') != '' && $this->request->get('dimension_parent_id') > 0) {
            array_push($rules['dimension_parent_id'], Rule::exists('first_dimensions', 'id'));
        } else {
